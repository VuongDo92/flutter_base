...
class WebRouterInterceptor with RouterInterceptor {
  ...
  bool canHandleRoute(
    ...
    if (uri.scheme == 'https' || uri.scheme == 'http') {
      final onlyAllowWebDomain =
          transitionSettings?.onlyAllowWebDomain ?? true;
...
//        for (var domain in env.rovoDomains) {
...
class StaticPagesRouterInterceptor extends WebRouterInterceptor {
  ...
  bool canHandleRoute(
    ...
    return canHandle &&
        (uri.path.startsWith('/p/') || uri.toString().contains('blog.rovo.co'));
  }
}
